...
Для v1 во всех четырёх platform lifecycle profiles обязательны common
[`modularity.md`](workflow/rules/system-design/modularity.md) и platform
modularity addendum. Новые cohesive feature/data/network/storage/reusable UI
capabilities по strong default получают physical platform unit; application
shell содержит только entry/lifecycle/root routing/DI/config/resources и
composition. Folder/layer не является module. Каждый `design.md` фиксирует
`isolated | deviation | not-applicable` и `PASS` boundary guard; deviation без
точного repository constraint, typed seam и objective migration trigger
блокируется.
Versionless v0 допустим только при exact hash match tracked
`workflow/compatibility/modularity-v0.json`; он завершает historical checks без
ретроактивного v1 composition verdict и без расширения ownership/structure.
