...
|---|---|---|---|
| `archive` | [`workflow/phases/archive.md`](workflow/phases/archive.md) | — | `platform archive package and ARCHIVED.md tombstone`, `product archive package and spec.md tombstone` |
...
| Platform implementation | `<platform>/specs/` и adapter-owned production root | plan, code, evidence, archive |
...
specs/product/<feature>/
  ...
  spec.md          # READY/APPROVED REQ↔AC contract
...
перемещает полный product package и оставляет в active path только `spec.md`
tombstone; `archive-receipt.json` создаёт только platform implementation
...
не требуют его.
Tasks образуют DAG, называют точные paths, Inline contract context и focused evidence.
Semantic adapter projection не хранится в plan snapshot: она вычисляется и
...
lint. Он не stage/commit/push. `deep-code-review` оборачивается private ephemeral
...
task/plan, а unsafe/mixed ownership route блокируется. Private guard сохраняет index,
production, shared/proposal/rules, historical evidence и unrelated dirty state;
...
user intent → explicit paths → reconcile per platform/feature/change → report
            → scoped stage → pre-commit-check → tracked hook → commit
```
...
| [`Android/workflow/rules/architecture/modularization.md`](Android/workflow/rules/architecture/modularization.md) | Modularization |
...
| [`iOS/workflow/rules/package-development.md`](iOS/workflow/rules/package-development.md) | Swift package development |
