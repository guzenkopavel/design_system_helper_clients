# reconciliation task-005 — android task-drift

- Result: PASS

## Итог

Сверка перед commit подтвердила, что интеграционные и доступностные тесты
закрывают вход, регистрацию, восстановление сессии, expiry, сетевую политику,
доступность и тему. `AppShellIntegrationTest.kt` добавлен в эту задачу, потому
что после появления auth gate он проверяет оболочку напрямую.
