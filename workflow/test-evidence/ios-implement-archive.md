...
> below about Android adapter absence, CLI `NOT IMPLEMENTED` results and
> synthetic Android archive fixtures record the pre-Android-adapter state; they
> are not current capability claims. Android now supports the full `propose →
> plan → implement → verify → archive` lifecycle. Current evidence:
> [`android-propose-plan-implement.md`](android-propose-plan-implement.md) and
> [`android-verify-archive.md`](android-verify-archive.md). The original outputs
> remain unchanged as historical iOS RED/GREEN evidence.
...
## GREEN — historical implementation snapshot
...
7. Historical pre-Android-adapter invocation: Android/unknown implementation
   invocation observed an absent adapter, поэтому
   resolver возвращает `NOT IMPLEMENTED` до package discovery и writes.

Historical CLI blockers (pre-Android adapter): Android `implement` и implementation archive —
