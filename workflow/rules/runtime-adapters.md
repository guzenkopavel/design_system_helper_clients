...
или task-coverage правил. Pre-tool `git commit` выполняет non-consuming receipt
preview. Независимо от runtime, финальная обязательная граница — tracked
[`.githooks/pre-commit`](../../.githooks/pre-commit), атомарно потребляющий
one-shot receipt и повторяющий staged gate. Наличие tracked hook не означает его
