...
`python3 workflow/scripts/pre-commit-check.py --staged`. Разрешение действует
только для того staged fingerprint, который вернул `PASS`; `FAIL` и `UNKNOWN`
останавливают commit. `--no-verify` запрещён. Полный контракт:
