...
Rename identity включает mutable old/new. Copy identity применяется только
когда coordinator явно указывает source+destination или Git сообщает copy;
обычный added file не становится copy только из-за совпадения blob с tracked
HEAD-файлом. Для явного copy source остаётся read-only unchanged, destination —
mutable, и обе стороны обязательны в intended/receipt. Missing intended или
extra foreign staged path дают `FAIL`, потому что commit включает весь index.
...
active task содержит discovered command и staged result evidence. Для
post-archive verified receipt trail terminal archive evidence удовлетворяет
project/tool obligation без повторного staged task result.
