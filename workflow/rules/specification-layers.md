...
- `review-verdicts.json` — coordinator-owned receipt шести independent lens
  outputs на одном fingerprint; reviewer его не пишет.
...
Для каждой линзы отдельный fresh `product-spec-reviewer` возвращает exact JSON;
coordinator хранит runtime invocation evidence, а JSON лишь attests provenance
и не доказывает isolation. Current PASS/GAP/UNKNOWN findings живут только в
`review-verdicts.json`; non-green receipt остаётся durable, но блокирует fan-out. Spec содержит
статические ссылки на receipt и UX readiness, не копирует verdict table.
...
Approval входит в fingerprint, поэтому после него все шесть final lenses
запускаются заново. Только exact Status metadata исключена из freshness:
остальные edit/add/delete package changes делают receipt stale.
...
1. **`product-backed`** — изменение реализует или меняет observable product
   ...
   статусами `READY`, `Product approval: APPROVED` и fresh green product review
   receipt, подтверждённым `validate-product-spec.py check`.
