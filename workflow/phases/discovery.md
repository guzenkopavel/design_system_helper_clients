...
1. **Current baseline:** если существует
   `specs/product/<feature>/SPECIFICATION.md`, прочитать его как immutable
   доставленный контракт и перечислить сохраняемое/изменяемое поведение. Не
   редактировать baseline из Discovery и не включать его в active review package.
2. **Frame:** сформулировать проблему, пользователей, текущее трение, outcome и
   признаки успеха без преждевременного выбора реализации.
3. **Evidence:** отделить факты от допущений; для рискованных допущений указать
   источник или способ проверки. Волатильные внешние ограничения фиксировать,
   не проектируя здесь платформенную архитектуру.
4. **Explore:** использовать существующий concept либо провести сокращённый
   brainstorming с 2–3 вариантами. Зафиксировать выбранное направление и
   отклонённые альтернативы с причинами.
5. **Shape:** описать scope, non-goals, общее observable behavior, основные и
   вторичные состояния: loading, empty, error, offline, accessibility,
   localization, analytics и privacy — только когда применимо.
6. **Draft UX impact:** для UI/interaction scope собрать достаточный вход для
   ...
   дизайн-системы. Зафиксировать default calm/soft-blue mood через semantic
   roles, quiet neutral surfaces, light/dark/high-contrast и non-color cues по
   [`visual-language.md`](../rules/visual-language.md). Не выбирать SDK,
   Liquid Glass/Material APIs, architecture или platform components.
7. **Parity pass:** отдельно проверить ожидаемое поведение iOS и Android.
...
8. **Draft contract:** создать требования `REQ-N` и observable критерии `AC-N`.
   Каждый AC содержит `Covers: REQ-N`; пробелы разрешены на этой фазе, но явно
   помечаются как draft gaps.
9. Записать `specs/product/<feature>/brief.md` по
