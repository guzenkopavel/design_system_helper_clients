...
→ Status READY → validate-product-spec.py check → STOP
```
...
| `archive implementation` | fresh verified package | immutable archive, receipt, active tombstone | `archived` |
...
ставит точный `PASS`, `FAIL` или `UNKNOWN`. При non-PASS coordinator также
...
явная просьба commit
  ...
  → $pre-commit-check по уже staged index
  → stable staged fingerprint: PASS
  → tracked Git hook повторяет тот же gate
  → commit в пределах исходного разрешения
```

`$pre-commit-check` не делает `git add`, commit или push. Любое изменение index
инвалидирует результат. Runtime hooks дают ранние deny/warnings, но не заменяют
tracked Git hook и не расширяют delivery authorization.
