...
READY требует coherent exact decision, atomic AC/evidence dimensions и Russian
authored product prose/review JSON через общий language helper.
Feature-root `SPECIFICATION.md` хранит доставленный current baseline отдельно от
active package: Discovery/Elaborate читают его, а product review fingerprint не
включает. Implementation archive симметрично публикует platform baseline для
следующих Propose/Plan, оставляя change archive immutable.
...
Guard хранит scoped projection выбранной identity и принимает disjoint dirty/
index/commit другой lane; selected intended paths, package, shared/rules/
adapter/control plane остаются guarded. Cross-package owner overlap блокируется.
...
resolver используется retrieval, validator и fingerprint logic. Plan разделяет
writable production `Paths` и immutable `Read-only context`; Implement получает
write authority только из `Paths`. Scope baseline schema v3 использует scoped
git-visible lane projection и exact selected index, не fingerprint'ит ignored
caches/build или disjoint work. Условные
delivery/DX rules не дают права коммитить или менять ветки.

Product retirement request по default path живёт в
`specs/product/_retirement-requests/` и при apply копируется в archive как
durable evidence, не меняя platform archives.
Product `completed` публикует approved baseline; `superseded`/`cancelled`
сохраняет прежний и не выдает retired candidate за текущее поведение.

Current v1 product-backed UI Verify использует общий exact `NATIVE-*` obligation set и
structured JSON observation records; platform addenda выбирают только native
commands/infrastructure. Non-PASS native ID переоткрывает sealed `ui` tasks и
dependent closure. Registry-anchored v0 не получает language/native retrofit и
завершает только historical exact-hash checks.
...
Coordinator gate связывает staged index с exact intended `--path` set; tracked
hook запускает one-shot receipt consumption, а runtime hook делает только
non-consuming preview. Generic hook без receipt не авторизует commit. Unrelated
unstaged state допустим, а extra/missing staged path и ambiguous package owner
дают FAIL.
Для rename identity и mutable set содержат old/new. Для copy intended/receipt
содержат explicit unchanged source и destination, но write ownership/task trail
проверяются только для destination; source остаётся guarded read-only.
